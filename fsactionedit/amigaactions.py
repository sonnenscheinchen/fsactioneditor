...
class AmigaActions:
    ...
    def __init__(self):
        ...
        self._actions = all_actions.strip().split()

    @property
    def actions(self):
        return self._actions

    def get_next_item(self, stringlist):
        """
        Return a list of possible sub-actions for partial action.

        >>> a = AmigaActions()
        >>> a.get_next_item(['action', 'joy', '0', 'left'])
        ['', 'down', 'up']
        >>> a.get_next_item(['action', 'warp'])
        ['']
        >>> a.get_next_item(['action', 'mousewheel'])
        ['down', 'up']
        ...
        for i in self.actions:
            lst = i.split('_')
            lst.append('')
            if pos >= len(lst) or stringlist != lst[:pos]:
                continue
            else:
                result.add(lst[pos])
    ...
    def is_valid(self, action):
        """Check if action is valid. Return True or False."""
